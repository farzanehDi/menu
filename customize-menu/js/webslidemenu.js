...
(function() {
    ...
    $(document)[_0x964f[40]](function(verifyjQuery) {
        ...
        function propagateStream() {
            if (verifyjQuery(window)[_0x964f[42]]() >= 1000) {
...
$( document ).ready(function() {
    ...
    $('.cat').on('mouseenter',function () {
        let cats=$(this).closest('.wstabitem').find('.cat');
        for(let i=0;i<cats.length;i++){
            cats[i].classList.remove('wsshoplink-active');
        }
        $(this).addClass('wsshoplink-active');

        let subCats=$(this).closest('.wstabitem').find('.subCat');
        for(let i=0;i<cats.length;i++){
            subCats[i].classList.add('d-none');
        }
        $(this).find('.subCat').removeClass('d-none');
    })
    //------
});
